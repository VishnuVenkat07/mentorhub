...
// ─── Internship Row ────────────────────────────────────────────────────────────
function InternRow({ item, onApply }) {
  ...
  return (
    <div style={{ background:'#f8fafc', minHeight:'100vh' }}>
      ...
      <section className="pb-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          <div className="mb-6">
            <h2 className="text-xl font-black" style={{ color:'#0f172a' }}>Internship Opportunities</h2>
            <p className="text-sm mt-1" style={{ color:'#64748b' }}>
              Hands-on internships with mentorship, real projects, and possible full-time conversion.
            </p>
          </div>
          <div className="flex flex-col gap-3">
            {internships.map(item => <InternRow key={item.id} item={item} onApply={setApplying} />)}
          </div>
        </div>
      </section>
